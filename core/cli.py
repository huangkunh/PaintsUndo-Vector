...
from core.painting_sim import sort_strokes_human_like
...
示例:
  ...
  paints-undo-vector --input photo.png --output result/ --device cuda:0
    ...
    parser.add_argument("--device", "-d", type=str, default=None, help="计算设备 (cuda/cpu)")
    parser.add_argument("--brush", "-b", type=str, default=None, help="使用的笔刷（逗号分隔）")
    parser.add_argument("--stage", "-s", type=int, default=None, help="仅运行指定阶段 (1/2/3)")
    parser.add_argument("--num-strokes", "-n", type=int, default=None, help="笔画数量覆盖")
    parser.add_argument("--iterations", type=int, default=None, help="迭代次数覆盖")
    parser.add_argument("--resolution", type=int, default=None, help="分辨率覆盖")
    ...
    parser.add_argument("--save-iterations", action="store_true", help="保存迭代中间结果")
...
def main():
    ...
    print(f"Target image loaded: {args.input} ({canvas_size[0]}x{canvas_size[1]})")
    ...
    if stages_config:
        for sc in stages_config:
            stage_configs.append(StageConfig(
                name=sc.get("name", ""),
                num_strokes=sc.get("num_strokes", 20),
                ...
                num_iterations=sc.get("num_iterations", 500),
                lr=sc.get("lr", 1.0),
                lr_width=sc.get("lr_width", 0.1),
                lr_color=sc.get("lr_color", 0.01),
                brush_names=sc.get("brush_names", ["marker"]),
        ...
        from core.optimizer import DEFAULT_STAGE_CONFIGS
        stage_configs = DEFAULT_STAGE_CONFIGS
    
    # 应用命令行覆盖
    if args.brush:
        brush_list = args.brush.split(",")
        for sc in stage_configs:
            sc.brush_names = brush_list
    
    if args.num_strokes:
        for sc in stage_configs:
            sc.num_strokes = args.num_strokes
    
    if args.iterations:
        for sc in stage_configs:
            sc.num_iterations = args.iterations
    
    if args.resolution:
        for sc in stage_configs:
            sc.resolution = (args.resolution, args.resolution)
